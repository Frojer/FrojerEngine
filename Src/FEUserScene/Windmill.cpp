...
void Windmill::Update()
{
	...
	if (m_pSystem->m_bDrawNormal)	pDebug->DrawNormal(m_pWindmillBody, FEVector4(0.0f, 1.0f, 0.0f, 1.0f));

	switch (m_type)
	{
	case WINDMILL_NORMAL:
		m_pWindmillWing[0]->GetTransform()->Rotate(FEVector3::Forward * -180.0f * FETime::GetDeltaTime() * m_pSystem->m_cold);
		if (m_pSystem->m_bDrawNormal)	pDebug->DrawNormal(m_pWindmillWing[0], FEVector4(0.0f, 1.0f, 0.0f, 1.0f));
		break;
	...
	case WINDMILL_TRIPLE_TYPE1:
		...
		if (m_pSystem->m_bDrawNormal)	for (int i = 0; i < 3; i++)		pDebug->DrawNormal(m_pWindmillWing[i], FEVector4(0.0f, 1.0f, 0.0f, 1.0f));
		break;
